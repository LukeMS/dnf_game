"""Scrollable Formatted Text for pygame (sftext).
...
if __name__ == '__main__':
    ...
    font_path = os.path.join('resources')
    sftext = SFText(text=text, font_path=font_path, style=mystyle)
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key.keysym.sym == pygame.K_ESCAPE:
                        alive = False
                    else:
