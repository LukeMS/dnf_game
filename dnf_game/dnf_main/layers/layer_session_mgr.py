...
from dnf_game.dnf_main.map_entities import PCreature, Cursor
from dnf_game.dnf_main.map_gen import rnd_gen, populate_level
from dnf_game.dnf_main.map_containers import MapContainer, MapHeader
from dnf_game.scene_manager.layers.base_layers import LogicLayer
from dnf_game.util import tile_fx, packer, dnf_path


def _save_path():
    """Utility to return the save path."""
    return os.path.join(dnf_path(), 'save')


def clear_savedir():
    """Clear a previous session."""
    if os.path.isdir(_save_path()):
        for root, path, files in os.walk(_save_path()):
            for f in files:
                os.remove(os.path.join(root, f))
        os.removedirs(_save_path())
...
class SessionMgr(LogicLayer):
    ...
    def load_game(self):
        ...
        with packer.zshelf_open(
                os.path.join(_save_path(), 'savegame'),
                'r') as shelf_file:
