    ...
    def on_event(self):
        ...
        for event in sdl2.ext.get_events():
            ...
            if event.type == sdl2.SDL_KEYUP:
                scene.on_key_release(event, modifiers)
            elif event.type == sdl2.SDL_KEYDOWN:
                scene.on_key_press(event, modifiers)
                ...
                if buttons & sdl2.SDL_BUTTON_LMASK:
                    scene.on_mouse_drag(x, y, dx, dy, "LEFT")
                elif buttons & sdl2.SDL_BUTTON_MMASK:
                    scene.on_mouse_drag(x, y, dx, dy, "MIDDLE")
                elif buttons & sdl2.SDL_BUTTON_RMASK:
                    scene.on_mouse_drag(x, y, dx, dy, "RIGHT")
                else:
